...
CClosure *luaF_newCclosure (lua_State *L, int nupvals) {
  int o = luaC_newobj(L, LUA_VCCL, sizeCclosure(nupvals));
...
LClosure *luaF_newLclosure (lua_State *L, int nupvals) {
  int o = luaC_newobj(L, LUA_VLCL, sizeLclosure(nupvals));
...
void luaF_initupvals (lua_State *L, LClosure *cl) {
  ...
  for (i = 0; i < cl->nupvalues; i++) {
    int o = luaC_newobj(L, LUA_VUPVAL, sizeof(UpVal));
    UpVal *uv = gco2upv(o);
...
Proto *luaF_newproto (lua_State *L) {
  int o = luaC_newobj(L, LUA_VPROTO, sizeof(Proto));
  Proto *f = gco2p(o);
  f->k = NULL;
  f->sizek = 0;
  f->p = NULL;
