...
/// 合并 find_local_ctc + find_local_ex + find_named_global_decl + is_undeclared_global(当前作用域)
/// 为单次线性扫描, 消除 3-4 次重复迭代 + 重复的 active/hash/name 比较。
...
enum LocalLookup {
    ...
    Undeclared,
    /// 当前作用域未找到 (无 global 声明或仅有 global *) — 需要查 upvalue
    NotFound,
}
...
impl Hash for ConstKey {
    fn hash<H: Hasher>(&self, state: &mut H) {
        ...
        // 导致 const_k 从 0.51% 暴增到 16.09%)。预混合后 FxHash 仅做 1 次 write_u64,
        // 且不同值的 hash 位分布均匀。SipHash 也受益 (减少 write 调用次数)。
        // hash_one (ConstKey) 占 2.21%, 切换 FxHash + 预混合后预期降至 < 0.5%。
        let h = match self {
            ConstKey::Nil => 0u64,
            ConstKey::Boolean(b) => 0x100 | (*b as u64),
            ConstKey::Integer(i) => {
                let v = *i as u64;
...
pub struct FuncState<'a> {
    ...
    /// 极少见), 首次查找后缓存, 后续直接复用。
    /// 失效: add_local/add_local_kind 中新 local 名为 "_ENV" 时清空。
    /// 不缓存 Ctc/NamedGlobal (极少见, 每次查)。
    env_cache: EnvCache,
    #[cfg(debug_assertions)]
...
/// _ENV 查找结果的缓存 (避免重复扫描 locals + find_upvalue)
#[derive(Clone, Copy)]
enum EnvCache {
    /// 未缓存 (首次查找前 / 失效后)
    None,
...
impl<'a> FuncState<'a> {
    ...
    fn error(&mut self, msg: &str) {
        self.errors.push(format!(
            "{}:{}: {}",
            crate::compiler::lexer::format_chunk_id(&self.ls().chunk_name),
            self.ls().lastline,
            msg
        ));
    }
...
impl<'a> FuncState<'a> {
    ...
    fn emit(&mut self, ins: Instruction) -> i32 {
        ...
        // 编译期间 proto.code 独占 (refcount=1), get_mut 总是成功。
        // 用 match 而非 unwrap_or_else 避免闭包借用冲突。
        let code = match Rc::get_mut(&mut self.proto.code) {
            Some(v) => v,
            None => Rc::make_mut(&mut self.proto.code),
        };
        code.push(ins);
    ...
    fn const_k(&mut self, value: TValue) -> i32 {
        ...
        match self.const_index.entry(key) {
            ...
            std::collections::hash_map::Entry::Vacant(e) => {
                ...
                // perf: Rc::get_mut 非原子检查, 编译期独占 (见 emit 注释)
                let constants = match Rc::get_mut(&mut self.proto.constants) {
                    Some(v) => v,
                    None => Rc::make_mut(&mut self.proto.constants),
                };
    ...
    fn free_exp_reg(&mut self, e: &ExpDesc) {
        if matches!(e.kind, ExpKind::NonReloc | ExpKind::Relocable)
            && (e.info as i32) >= self.nvarstack()
            && (e.info as i32) == self.freereg - 1
        {
            self.free_reg();
        }
    }
    ...
    fn add_local(&mut self, name: &str, start_pc: i32) -> i32 {
        ...
        let name_ls = self.ls_mut().anchor_string(name);
        // 失效 _ENV 缓存: 新声明 `local _ENV` 会改变 _ENV 的 resolution
        if name == "_ENV" {
            self.env_cache = EnvCache::None;
        }
        let varname = Some(name_ls.clone());
    ...
    fn find_local_ctc(&mut self, name: &LuaString) -> Option<ExpDesc> {
        ...
        for lv in self.locals.iter().rev() {
            ...
            // hash 不匹配 → 内容必然不同 → 跳过 (inline 比较,零 Rc deref)
            if lv.name_hash != name_hash {
                continue;
            }
            // hash 匹配 (极少数情况): 做完整比较确认
            if lv.name == *name {
                // 找到首个匹配（影子规则）：非 RDKCTC 立即返回 None
                if lv.kind != RDKCTC {
                    return None;
                }
                // RDKCTC：根据 ctc_kind 返回相应 ExpDesc
                let kind = lv.ctc_kind.clone().unwrap();
                if kind == ExpKind::Str {
                    if let Some(ref s) = lv.ctc_str {
                        return Some(ExpDesc::new_str(s.clone()));
                    }
                }
                return Some(ExpDesc::new(kind, lv.ctc_info.unwrap()));
            }
        }
        None
    }

    /// 一次扫描 locals, 合并 find_local_ctc + find_local_ex + find_named_global_decl
    /// + is_undeclared_global(当前作用域) 的完整查找。
    ...
    fn lookup_local(&mut self, name: &LuaString) -> LocalLookup {
        ...
        for lv in self.locals.iter().rev() {
            ...
            if lv.kind >= GDKREG {
                ...
                // regular local (RDKREG/RDKCONST/RDKTOCLOSE/RDKCTC/RDKVAVAR)
                if lv.name_hash != name_hash {
                    continue;
                }
                if lv.name != *name {
                ...
                if lv.kind == RDKCTC {
                    ...
                    if kind == ExpKind::Str {
                        if let Some(ref s) = lv.ctc_str {
                            return LocalLookup::Ctc(ExpDesc::new_str(s.clone()));
                        }
                    }
    ...
    /// 解析 _ENV 的位置, 带缓存。
    /// 返回 (is_local, local_reg, local_kind, upval_idx):
    /// - is_local=true: _ENV 是 local, 用 local_reg/local_kind
    /// - is_local=false, upval_idx>=0: _ENV 是 upvalue idx
    /// - is_local=false, upval_idx<0: _ENV 是 implicit upvalue 0 (顶层函数)
    ...
    fn resolve_env(&mut self, env_ls: &LuaString) -> EnvResolution {
        ...
        match self.env_cache {
            EnvCache::Upvalue(idx) => {
                return EnvResolution::Upvalue(idx);
            }
            EnvCache::Implicit => {
                return EnvResolution::Implicit;
            }
            _ => {}
        }
            ...
            LocalLookup::GlobalStar | LocalLookup::Undeclared | LocalLookup::NotFound => {
                ...
                match self.find_upvalue(env_ls) {
                    Some(UpvalueOrCtc::Upvalue(idx)) => {
                        self.env_cache = EnvCache::Upvalue(idx);
                        EnvResolution::Upvalue(idx)
                    }
                    _ => {
                        self.env_cache = EnvCache::Implicit;
                        EnvResolution::Implicit
                    }
                }
            }
        }
    ...
    fn find_local_ex(&self, name: &LuaString) -> Option<(i32, i32)> {
        ...
        for lv in self.locals.iter().rev() {
            ...
            // hash 不匹配 → 内容必然不同 → 跳过 (inline 比较,零 Rc deref)
            // 例外: global * 的名字是 "(global *)",hash 不会匹配用户变量,
            // 所以 global * 仍然会被正确跳过 (C 的 searchvar 也不匹配 global *)
            if lv.name_hash != name_hash {
                continue;
            }
            if lv.kind >= GDKREG {
    ...
    fn find_global_decl(&self, name: &LuaString) -> Option<i32> {
        ...
        for lv in self.locals.iter().rev() {
            ...
            if lv.kind >= GDKREG {
                // global declaration: must process even if hash doesn't match,
                // because global * sets collective_kind for all variables
                if lv.is_global_star {
                    if info < 0 {
                        info = 0;
                ...
                // regular local: hash 预比较快速淘汰不匹配的 local
                if lv.name_hash == name_hash && lv.name == *name {
                    return None;
                }
            }
        }
        if info >= 0 {
            Some(collective_kind)
        } else {
            None
        }
    }
    ...
    fn is_undeclared_global(&self, name: &LuaString) -> bool {
        ...
        self.is_undeclared_global_parent(name)
    }
...
fn get_name(fs: &mut FuncState) -> LuaString {
    match &fs.ls().token {
        ...
        _ => {
            ...
            // 返回空字符串作为错误恢复的占位符 (对应 C 返回空 TString)
            fs.ls_mut().anchor_string("")
        }
    }
}
...
fn parse_chunk(fs: &mut FuncState) {
    ...
    fs.block_stack.push(BlockEntry {
        saved_nlocals,
        saved_ngotos,
        has_upval: false,
        is_function_body: true,
        nactvar: entry_nactvar,
        reglevel: entry_reglevel,
        insidetbc: false,
    });
    ...
    let has_tbc = fs.locals[saved_nlocals..]
        .iter()
...
fn close_func(fs: &mut FuncState) {
    ...
    if let Some(pb) = fs.pending_func_block.take() {
        ...
        solve_gotos_for_block(
            fs,
            pb.saved_nlabels,
            pb.saved_nlocals,
            pb.saved_ngotos,
            pb.has_tbc || pb.has_upval,
            pb.nactvar,
            pb.reglevel,
        );
...
fn parse_block(fs: &mut FuncState) {
    ...
    solve_gotos_for_block(
        ...
        saved_nlabels,
        saved_nlocals,
        block_entry.saved_ngotos,
        has_tbc || has_upval,
        block_entry.nactvar,
        block_entry.reglevel,
    );
}
...
fn checkglobal(fs: &mut FuncState, varname: &str, line: i32) {
    ...
    fs.fixline(line);
    let k_bx = if k >= crate::opcodes::MAXARG_BX as i32 {
        0
    } else {
        k + 1
    };
...
fn globalnames(fs: &mut FuncState, defkind: i32) {
    ...
    if has_init {
        ...
        for i in 0..nvars {
            // names[i] 已是 intern 的 LuaString, 直接复用避免再次 anchor
            let k = fs.string_k_ls(names[i].clone());
            var_k_names.push(k);
        }
        ...
        let env_local_ex = fs.find_local_ex(&env_ls);
        let env_is_vvargvar = env_local_ex
            .map(|(_, kind)| kind == RDKVAVAR)
            .unwrap_or(false);
        ...
        for i in (0..nvars).rev() {
            ...
            if pe.table_reg >= 0 && !env_is_vvargvar {
                ...
                if env_is_local {
                    ...
                    let k_bx = if var_k_names[i] >= crate::opcodes::MAXARG_BX as i32 {
                        0
                    } else {
                        var_k_names[i] + 1
                    };
...
fn parse_statement(fs: &mut FuncState) {
    ...
    let result = match &fs.ls().token {
        ...
        Token::Break => {
            ...
            let reglevel = fs.reglevel_for_nactvar(nactvar);
            let nlocals = fs.locals.len();
            let break_name = fs.ls_mut().anchor_string("break");
            fs.gotos.push(GotoDesc {
                name: break_name,
                pc,
                line: fs.ls().lastline,
        ...
        Token::Name(name) => {
            ...
            let is_global = name == "global" && {
                ...
                matches!(
                    lk,
                    Token::Lt | Token::Name(_) | Token::Star | Token::Function
                )
            };
            if is_global {
        ...
        Token::LParen => {
            // 对应 C 的 exprstat: (f)() 调用 或 (t).field = 1 赋值
            parse_assign_or_call(fs);
            if check(fs, &Token::Semi) {
                fs.ls_mut().next();
            }
...
fn check_conflict_for_var(
    fs: &mut FuncState,
    prev_vars: &mut Vec<PrefixResult>,
    new_var: &PrefixResult,
) {
...
fn parse_assign_or_call(fs: &mut FuncState) {
    ...
    if !has_call
        && (check(fs, &Token::LParen)
            || check(fs, &Token::Colon)
            || check(fs, &Token::LBrace)
            || matches!(&fs.ls().token, Token::String(..)))
    {
        has_call = true;
    ...
    loop {
        match &fs.ls().token {
            Token::Dot => {
                ...
                // field 已是 intern 的 LuaString, 直接复用避免再次 anchor
                let k = fs.string_k_ls(field.clone());
                ...
                if first.is_vvargvar {
                    ...
                    first = PrefixResult {
                        ...
                        key: None,
                        reg: Some(base_reg),
                        table_reg: Some(base_reg),
                        table_key: Some(kr),
                        table_key_is_const: false,
                        table_key_is_int: false,
                ...
                first = PrefixResult {
                    ...
                    table_key_is_const: table_key_is_const,
                    table_key_is_int: false,
                    key_allocated_reg: key_allocated_reg,
                    allocated_reg: if new_is_upvalue {
                        false
                    } else {
                        first.allocated_reg
                            || first.reg.is_none()
                            || (first.is_upvalue && !is_short_str)
                    },
                    is_upvalue: new_is_upvalue,
                    upval_idx: first.upval_idx,
                    env_gettabup_pc: if new_is_upvalue {
                        -1
                    } else {
                        if gettabup_pc >= 0 {
                            gettabup_pc
                        } else {
                            first.env_gettabup_pc
                        }
                    },
                    has_call: false,
                    call_pc: -1,
            ...
            Token::LBracket => {
                ...
                if first.is_vvargvar {
                    ...
                    let key_reg = if ei.exp.kind == ExpKind::NonReloc
                        && (ei.exp.info as i32) < fs.nvarstack()
                    {
                        ei.exp.info as i32
                    } else {
                        fs.exp_to_next_reg(&ei.exp)
                ...
                let getupval_emitted_before_key = is_upvalue_table
                    && !key_has_jumps
                ...
                let upval_key_placeholder_pc = if is_upvalue_table
                ...
                } else if matches!(ei.exp.kind, ExpKind::Relocable | ExpKind::NonReloc)
                    && !ei.exp.has_jumps()
                    && ei.exp.info2 < 0
                {
                    (ei.exp.info as i32, false, false)
                } else if is_upvalue_table
                    && upval_key_placeholder_pc.is_none()
                    && !getupval_emitted_before_key
                ...
                let (base_reg, new_is_upvalue, allocated_reg) = if is_upvalue_table {
                    ...
                    (
                        base_reg,
                        first.is_upvalue,
                        first.allocated_reg || first.reg.is_none(),
                    )
                };
                ...
                first = PrefixResult {
                    ...
                    table_reg: Some(base_reg),
                    table_key: Some(kr),
                    table_key_is_const: key_is_const,
                    table_key_is_int: key_is_int,
    ...
    if check(fs, &Token::Eq) || check(fs, &Token::Comma) {
        ...
        for v in &mut vars {
            if let Some(ref name) = v.var_name {
                if v.key.is_some() && !v.is_upvalue {
                    let k_name = fs.string_k_ls(name.clone());
        ...
        let last_is_call = exps
            .last()
            .map_or(false, |e| e.kind == ExpKind::Call && e.info2 >= 0);
            ...
            if e.has_jumps() {
                ...
                if e.info2 >= 0 {
                    if e.kind == ExpKind::Call {
                        ...
                        if prev_dest != dest
                            && (prev_dest >= fs.nvarstack() || prev_dest == fs.freereg - 1)
                        {
                            fs.free_reg();
                        }
                        fs.resolve_jumps(e, dest);
...
fn parse_func_args(fs: &mut FuncState, freg: i32, src_reg: Option<i32>) -> i32 {
    ...
    if matches!(&fs.ls().token, Token::String(..)) {
        ...
        fs.ls_mut().next();
        // str_s 已是 intern 的 LuaString, 直接复用避免再次 anchor
        let k = fs.string_k_ls(str_s);
        let kr = fs.alloc_reg();
        fs.code_loadk(kr, k);
    ...
    if check(fs, &Token::Colon) {
        ...
        // method 已是 intern 的 LuaString, 直接复用避免再次 anchor
        let k = fs.string_k_ls(method.clone());
...
fn parse_args(fs: &mut FuncState) -> (i32, bool) {
    ...
    if !last_is_call
        && !last_is_vararg
        && matches!(ei.exp.kind, ExpKind::NonReloc)
        && (ei.exp.info as i32) < fs.nvarstack()
        && !ei.exp.has_jumps()
    {
    ...
    while check(fs, &Token::Comma) {
        ...
        let _r2 = if last_is_call || last_is_vararg {
            -1
        } else if matches!(ei2.exp.kind, ExpKind::Relocable)
            && ei2.exp.info2 >= 0
            && !ei2.exp.has_jumps()
        {
            fs.exp_to_reg(&ei2.exp)
        } else if matches!(ei2.exp.kind, ExpKind::Relocable | ExpKind::NonReloc)
            && !ei2.exp.has_jumps()
        {
            if ei2.exp.info2 >= 0 {
        ...
        if !last_is_call
            && !last_is_vararg
            && matches!(ei2.exp.kind, ExpKind::NonReloc)
            && (ei2.exp.info as i32) < fs.nvarstack()
            && !ei2.exp.has_jumps()
        {
            let target = fs.alloc_reg();
...
fn code_global_via_env_prefix(fs: &mut FuncState, name: &str) -> PrefixResult {
    ...
    // 缓存命中时 O(1) 直接返回 Local/Upvalue/Implicit, 跳过 locals 扫描 + find_upvalue。
    let env_resolution = fs.resolve_env(&env_ls);
    // C's buildglobal raises "_ENV is global when accessing variable 'X'" when
    // _ENV itself is a named global declaration.
    if matches!(env_resolution, EnvResolution::NamedGlobal) {
        fs.error(&format!(
    ...
    match env_resolution {
        EnvResolution::Ctc(mut env_ctc) => {
            ...
            discharge_ctc_to_reg(fs, &mut env_ctc, env_r);
            if is_short_str {
                PrefixResult {
                    var_name: Some(name_ls.clone()),
                    local_idx: None,
                    key: None,
                    reg: None,
                    table_reg: Some(env_r),
                    table_key: Some(k),
                    table_key_is_const: true,
                    table_key_is_int: false,
                    key_allocated_reg: false,
                    allocated_reg: true,
                    is_upvalue: false,
        ...
        EnvResolution::Local { reg: env_reg, kind } => {
        ...
        EnvResolution::Upvalue(env_upval_idx) => {
            // _ENV is an upvalue captured from parent.
            PrefixResult {
                var_name: Some(name_ls.clone()),
                local_idx: None,
                key: Some(k),
        ...
        EnvResolution::NamedGlobal => {
            // 上面已报错, 降级到 implicit upvalue 0
            PrefixResult {
                var_name: Some(name_ls.clone()),
                local_idx: None,
                key: Some(k),
        ...
        Token::Name(name) => {
            ...
            match lookup {
                LocalLookup::Ctc(mut ctc) => {
                    // CTC: discharge 常量到寄存器
                    let r = fs.alloc_reg();
                    discharge_ctc_to_reg(fs, &mut ctc, r);
                    PrefixResult {
                ...
                LocalLookup::GlobalStar | LocalLookup::Undeclared | LocalLookup::NotFound => {
                    if let Some(result) = fs.find_upvalue(&name) {
                        ...
                        // upvalue 未找到: 检查 undeclared
                        // Undeclared: 当前作用域有非匹配 named global 无 global * → is_undeclared_global 返回 true
                        // GlobalStar/NotFound: 当前作用域无 undeclared, 但 parent 可能有 → 检查 parent
                        if matches!(lookup, LocalLookup::Undeclared) {
                            fs.error(&format!("variable '{}' not declared", name));
                        } else if fs.is_undeclared_global_parent(&name) {
                            fs.error(&format!("variable '{}' not declared", name));
                        }
        ...
        match &fs.ls().token {
            Token::Dot => {
                ...
                if result.table_reg.is_some() {
                    ...
                    if result.is_upvalue {
                        // upvalue 索引表达式 (VINDEXUP)：discharge 到寄存器
                        // 生成 GETTABUP（短字符串 key）或 GETUPVAL+GETI/GETTABLE（其他 key）
                        ...
                        let r = fs.alloc_reg();
                        if result.table_key_is_const && !result.table_key_is_int {
                            code_gettabup(fs, r, prev_table, prev_key);
                        } else if result.table_key_is_int {
                            fs.code_abc(OpCode::GETUPVAL, r, prev_table, 0);
                            fs.code_abc(OpCode::GETI, r, r, prev_key);
                        } else {
                            // prev_key 是寄存器（长字符串或其他表达式）
                            fs.code_abc(OpCode::GETUPVAL, r, prev_table, 0);
                            fs.code_abc(OpCode::GETTABLE, r, r, prev_key);
                        }
                        result.reg = Some(r);
                        result.allocated_reg = true;
                        result.is_upvalue = false;
                    } else if result.table_key_is_int {
                        if result.allocated_reg {
                            fs.free_reg();
                        }
                ...
                result = PrefixResult {
                    ...
                    allocated_reg: if new_is_upvalue {
                        ...
                        result.allocated_reg
                            || result.reg.is_none()
                            || (result.is_upvalue && !is_short_str)
                    },
                    is_upvalue: new_is_upvalue,
            ...
            Token::LBracket => {
                ...
                let (base_reg, new_is_upvalue, allocated_reg) = if is_upvalue_table {
                    ...
                    (
                        base_reg,
                        result.is_upvalue,
                        result.allocated_reg || result.reg.is_none(),
                    )
                };
                // Now resolve deferred key register allocation (Upval/Relocable/Str key for upvalue table)
                let (kr, key_allocated) = if kr == -1 && is_upvalue_table {
            ...
            Token::LParen | Token::LBrace | Token::String(..) | Token::Colon => {
                ...
                } else if let Some(reg) = result.reg {
                    // For non-call results (e.g., parenthesized expressions like
                    // (f)()), if the expression is already in a temp register at
                    // freereg-1, reuse it directly without a MOVE — matching C's
                    // luaK_exp2nextreg for VRELOC (patches instruction in place).
                    // Skip locals (reg < nvarstack): C always MOVEs locals to a
...
fn parse_subexpr(fs: &mut FuncState, limit: i32) -> ExprItem {
    ...
    loop {
        ...
        if limit <= PREC_AND && check(fs, &Token::And) {
            let mut e_left = std::mem::replace(&mut e.exp, ExpDesc::new(ExpKind::Void, 0));
            fs.ls_mut().next();
            ...
            if e_left.kind == ExpKind::VJMP && e_left.info != NO_JUMP as i64 {
                ...
                let skip_test = matches!(e_left.kind, ExpKind::Boolean if e_left.info != 0)
                ...
                if skip_test {
                    ...
                    let is_vreloc_not = e_left.kind == ExpKind::Relocable
                        && e_left.info2 >= 0
            ...
            // perf: 用 destructure 替代 clone, 避免 ExpDesc (40 字节) 的深拷贝。
            // e2 后续不再使用, 可直接 move 出 exp 字段。
            let ExprItem { exp: mut e2_exp } = e2;
        ...
        if limit <= PREC_COMP && check_compare(fs) {
            let mut ec = std::mem::replace(&mut e.exp, ExpDesc::new(ExpKind::Void, 0));
            ...
            let k = if matches!(op_tok, Token::TildeEq) {
                0
            } else {
                1
            };
            ...
            if let Some((sc_val, isfloat)) = sc_imm {
                ...
                let jmp_pc = fs.jump();
                if reg_alloc
                    || (matches!(ec.kind, ExpKind::NonReloc | ExpKind::Call | ExpKind::Vararg)
                        && reg >= fs.nvarstack())
                    || (matches!(
                        e2.exp.kind,
                        ExpKind::NonReloc | ExpKind::Call | ExpKind::Vararg
                    ) && reg >= fs.nvarstack())
                {
                    fs.free_reg();
                }
                e = ExprItem {
                    exp: ExpDesc::new(ExpKind::VJMP, jmp_pc as i64),
                };
            } else if is_gt {
                ...
                if is_eq {
                    ...
                    if let Some(k_idx) = ec_const_k {
                        ...
                        if matches!(ec.kind, ExpKind::NonReloc)
                            && (ec.info as i32) >= fs.nvarstack()
                            && (ec.info as i32) == fs.freereg - 1
                        {
                            fs.free_reg();
                        }
                        e = ExprItem {
                        ...
                        if let Some((sc_val, isfloat)) = is_sc_number(&e2.exp) {
                            ...
                            if let Some(k_idx) = const_k {
                                ...
                                let r2 = if r2_alloc {
                                    fs.exp_to_reg(&e2.exp)
                                } else if matches!(e2.exp.kind, ExpKind::Vararg)
                                    && !e2.exp.has_jumps()
                                {
        ...
        if limit <= PREC_BOR && check(fs, &Token::Pipe) {
            ...
            if v1_opt.is_some() && v2_opt.is_some() {
                ...
                if let Some(k) = k_idx {
                    ...
                    e = ExprItem {
                        exp: ExpDesc::new_reloc_with_pc(0, pc),
                    };
                } else {
                    ...
                    } else if matches!(first_ec.kind, ExpKind::Relocable)
                        && !first_ec.has_jumps()
                        && first_ec.info2 < 0
                    {
                        first_ec.info as i32
            ...
            if v1_opt.is_some() && shift_opt.is_some() {
                ...
                if matches!(ec.kind, ExpKind::Int) && fits_sc(&ec) && !ec.has_jumps() {
                    ...
                    let v2 = if matches!(e2.exp.kind, ExpKind::NonReloc) && !e2.exp.has_jumps() {
                        e2.exp.info as i32
                    } else if matches!(e2.exp.kind, ExpKind::Relocable)
                        && !e2.exp.has_jumps()
                        && e2.exp.info2 < 0
                    {
                        e2.exp.info as i32
                ...
                } else if matches!(e2.exp.kind, ExpKind::Int)
                    && fits_sc(&e2.exp)
                    && fits_sc_neg(e2.exp.info)
                    && !e2.exp.has_jumps()
                {
        ...
        if limit <= PREC_CONCAT && check(fs, &Token::DotDot) {
            ...
            e = ExprItem {
                exp: ExpDesc::new(ExpKind::Relocable, r as i64),
            };
            matched = true;
        }
        ...
        if limit <= PREC_ADD && check_addop(fs) {
            ...
            match (&ec.kind, &e2.exp.kind) {
                (ExpKind::Int, ExpKind::Int) if !ec.has_jumps() && !e2.exp.has_jumps() => {
                    let val = if is_add {
                        ec.info.wrapping_add(e2.exp.info)
                    } else {
                        ec.info.wrapping_sub(e2.exp.info)
                    };
                    e = ExprItem {
                        exp: ExpDesc::new(ExpKind::Int, val),
                    };
                ...
                (ExpKind::Float, ExpKind::Int) if !ec.has_jumps() && !e2.exp.has_jumps() => {
                    ...
                    if !val.is_nan() && val != 0.0 {
                        ...
                        if k <= 255 {
                            ...
                            e = ExprItem {
                                exp: ExpDesc::new_reloc_with_pc(r as i64, pc),
                            };
                        } else {
                ...
                _ => {
                    // Like C's codebinexpval: process e2 first, then e1
                    if !is_add
                        && matches!(e2.exp.kind, ExpKind::Int)
                        && fits_sc(&e2.exp)
                        && fits_sc_neg(e2.exp.info)
                        && !e2.exp.has_jumps()
                    {
                        ...
                        && !e2.exp.has_jumps()
                    {
                        ...
                        if k <= 255 {
                            ...
                            // Like C's codebinexpval: process e2 first, then e1
                            let v2 = if matches!(e2.exp.kind, ExpKind::NonReloc)
                                && !e2.exp.has_jumps()
                            {
                                let reg = e2.exp.info as i32;
                                if e2.exp.info2 >= 0 {
                    ...
                    } else if is_add && matches!(e2.exp.kind, ExpKind::Float) && !e2.exp.has_jumps()
                    {
                        ...
                        && matches!(e2.exp.kind, ExpKind::Float)
                        && !e2.exp.has_jumps()
                    {
                        let f = f64::from_bits(e2.exp.info as u64);
                        let k = fs.float_k(f);
            ...
            match (&ec.kind, &e2.exp.kind) {
                (ExpKind::Int, ExpKind::Int) if !ec.has_jumps() && !e2.exp.has_jumps() => {
                    if is_idiv {
                        ...
                        if denom != 0 {
                            let q = ec.info / denom;
                            let val = if (ec.info ^ denom) < 0 && ec.info % denom != 0 {
                                q - 1
                            } else {
                                q
                            };
                ...
                (ExpKind::Float, ExpKind::Int) if !ec.has_jumps() && !e2.exp.has_jumps() => {
                    ...
                    if is_mul {
                        ...
                        if n != 0 {
                            ...
                            let val = if val != 0.0 && (f.signum() as i64 * n.signum() as i64) < 0 {
                                val + n as f64
                            } else {
                                val
                            };
                ...
                (ExpKind::Int, ExpKind::Float) if !ec.has_jumps() && !e2.exp.has_jumps() => {
                    ...
                    if is_mul {
                        ...
                        if f != 0.0 {
                            ...
                            let val = m % f;
                            // C's luaV_modf sign correction
                            let val = if val != 0.0 && (m.signum() as i64 * f.signum() as i64) < 0 {
                                val + f
                            } else {
                                val
                            };
                ...
                (ExpKind::Float, ExpKind::Float) if !ec.has_jumps() && !e2.exp.has_jumps() => {
                    ...
                    if is_mul {
                        ...
                        if f2 != 0.0 {
                            let val = f1 % f2;
                            // C's luaV_modf sign correction
                            let val = if val != 0.0 && (f1.signum() as i64 * f2.signum() as i64) < 0
                            {
                                val + f2
                            } else {
                                val
                ...
                (ExpKind::Int, _) if is_mul => {
                    ...
                    if k <= 255 {
                        ...
                        if matches!(e2.exp.kind, ExpKind::NonReloc | ExpKind::Call)
                            && r2 >= fs.nvarstack()
                            && r2 == fs.freereg - 1
                        {
                            fs.free_reg();
        ...
        if limit <= PREC_POW && check(fs, &Token::Caret) {
            ...
            if foldable {
                ...
                if !result.is_nan() && result != 0.0 {
                    e = ExprItem {
                        exp: ExpDesc::new(ExpKind::Float, result.to_bits() as i64),
                    };
                } else {
                    pow_no_fold = true;
                }
...
fn check_compare(fs: &FuncState) -> bool {
    matches!(
        fs.ls().token,
        Token::EqEq | Token::TildeEq | Token::Lt | Token::LtEq | Token::Gt | Token::GtEq
    )
}
...
fn code_global_via_env(fs: &mut FuncState, name: &str) -> ExpDesc {
    ...
    let (r, pc) = match env_resolution {
        ...
        EnvResolution::Ctc(mut env_ctc) => {
            // _ENV is a compile-time constant (local _ENV <const> = ...).
            // discharge the constant to a register, then use GETFIELD.
            let env_r = fs.alloc_reg();
            discharge_ctc_to_reg(fs, &mut env_ctc, env_r);
            if is_short_str {
                let r = fs.alloc_reg();
...
/// 处理 Token::Name 的 lookup 逻辑 (parse_simple_exp 中 Name 分支的提取)。
/// 提取为独立函数, 供 fast path (mem::replace 取出 String, 避免堆分配) 和
/// 原 match 中的 fallback (clone) 共用同一份逻辑。
///
...
fn parse_simple_exp_name(fs: &mut FuncState, name: LuaString) -> ExpDesc {
    ...
    match fs.lookup_local(&name) {
        LocalLookup::Ctc(ctc) => ctc,
        LocalLookup::Local { reg, kind } => {
            if kind == RDKVAVAR {
        ...
        // 需查 upvalue (parent_locals) 确认 parent 是否有覆盖或匹配
        LocalLookup::Undeclared => {
            if let Some(result) = fs.find_upvalue(&name) {
                match result {
                    UpvalueOrCtc::Upvalue(upval_idx) => ExpDesc {
        ...
        LocalLookup::NotFound => {
            if let Some(result) = fs.find_upvalue(&name) {
                ...
                let env_ls = fs.ls_mut().env_str_cached();
                let env_idx = match fs.find_upvalue(&env_ls) {
                ...
                // 当前作用域无 global 声明, parent 也无 upvalue:
                // is_undeclared_global 当前作用域返回 false (info == -1),
                // 但 parent 可能有非匹配 named global → 需检查 parent
...
fn parse_simple_exp(fs: &mut FuncState) -> ExprItem {
    ...
    // (perf 中 read_name malloc 1.66%)。matches! 返回 bool 不持有借用,
    // 所以 fs.ls_mut() 可立即获取可变借用。
        ...
        match &fs.ls().token {
            ...
            Token::Dot => {
                    ...
                    && !is_short_str
                {
                    ...
                    let result_reg = if matches!(e.kind, ExpKind::NonReloc)
            ...
            Token::LBracket => {
                ...
                if table_is_upvalue {
                    if ei.exp.kind == ExpKind::Str && !ei.exp.has_jumps() {
                        let k = fs.get_str_k(&ei.exp);
                        if is_kstr(fs, k) {
                ...
                }
                {
                        ...
                        && ei.exp.info <= ((1u32 << SIZE_C) - 1) as i64
                    {
                    ...
                    } else if ei.exp.kind == ExpKind::Str && !ei.exp.has_jumps() {
                        let k = fs.get_str_k(&ei.exp);
                        if is_kstr(fs, k) {
                            result_reg = if base_is_nonreloc_local {
                                fs.alloc_reg()
                        ...
                        let key_reg =
                            if matches!(ei.exp.kind, ExpKind::Relocable | ExpKind::NonReloc)
                                && !ei.exp.has_jumps()
                            {
                                if ei.exp.info2 >= 0 {
                                    if (ei.exp.info as i32) == fs.freereg - 1
                                        && (ei.exp.info as i32) >= fs.nvarstack()
                                    {
                                        fs.free_reg();
                                    }
                        ...
                        if base_is_nonreloc_local
                            && key_reg >= fs.nvarstack()
                            && key_reg == fs.freereg - 1
                        {
                            fs.free_reg();
                        }
                        result_reg = if base_is_nonreloc_local {
...
fn parse_repeat(fs: &mut FuncState) {
    ...
    let parent_insidetbc = fs.block_stack.last().map(|b| b.insidetbc).unwrap_or(false);
    fs.block_stack.push(BlockEntry {
        saved_nlocals: bl1_nlocals,
        saved_ngotos: bl1_saved_ngotos,
        has_upval: false,
        is_function_body: false,
        nactvar: bl1_entry_nactvar,
        reglevel: bl1_entry_reglevel,
        insidetbc: parent_insidetbc,
    });
    ...
    let parent_insidetbc = fs.block_stack.last().map(|b| b.insidetbc).unwrap_or(false);
    fs.block_stack.push(BlockEntry {
        saved_nlocals: bl2_nlocals,
        saved_ngotos: bl2_saved_ngotos,
        has_upval: false,
        is_function_body: false,
        nactvar: bl2_entry_nactvar,
        reglevel: bl2_entry_reglevel,
        insidetbc: parent_insidetbc,
    });
    ...
    let bl2_has_tbc = fs.locals[bl2_nlocals..]
        .iter()
        .any(|l| l.kind == RDKTOCLOSE && l.active);
    ...
    solve_gotos_for_block(
        ...
        bl2_nlabels,
        bl2_nlocals,
        bl2_entry.saved_ngotos,
        bl2_has_tbc || bl2_has_upval,
        bl2_entry.nactvar,
        bl2_entry.reglevel,
    );
    ...
    let bl1_has_tbc = fs.locals[bl1_nlocals..]
        .iter()
        .any(|l| l.kind == RDKTOCLOSE && l.active);
    ...
    solve_gotos_for_block(
        ...
        bl1_nlabels,
        bl1_nlocals,
        bl1_entry.saved_ngotos,
        bl1_has_tbc || bl1_has_upval,
        bl1_entry.nactvar,
        bl1_entry.reglevel,
    );
    fs.break_list = saved_breaklist;
...
fn parse_for(fs: &mut FuncState) {
    ...
    if check(fs, &Token::Eq) {
        ...
        let parent_insidetbc = fs.block_stack.last().map(|b| b.insidetbc).unwrap_or(false);
        fs.block_stack.push(BlockEntry {
            saved_nlocals: forstat_nlocals,
            saved_ngotos: forstat_saved_ngotos,
            has_upval: false,
            is_function_body: false,
            nactvar: forstat_entry_nactvar,
            reglevel: forstat_entry_reglevel,
            insidetbc: parent_insidetbc,
        });
        ...
        let parent_insidetbc = fs.block_stack.last().map(|b| b.insidetbc).unwrap_or(false);
        fs.block_stack.push(BlockEntry {
            saved_nlocals: body_nlocals,
            saved_ngotos: body_saved_ngotos,
            has_upval: false,
            is_function_body: false,
            nactvar: body_entry_nactvar,
            reglevel: body_entry_reglevel,
            insidetbc: parent_insidetbc,
        });
        ...
        let body_has_tbc = fs.locals[body_nlocals..]
            .iter()
            .any(|l| l.kind == RDKTOCLOSE && l.active);
        ...
        solve_gotos_for_block(
            ...
            body_nlabels,
            body_nlocals,
            body_entry.saved_ngotos,
            body_has_tbc || has_body_upval,
            body_entry.nactvar,
            body_entry.reglevel,
        );
        ...
        let forstat_has_tbc = fs.locals[forstat_nlocals..]
            .iter()
            .any(|l| l.kind == RDKTOCLOSE && l.active);
        ...
        solve_gotos_for_block(
            ...
            forstat_nlabels,
            forstat_nlocals,
            forstat_entry.saved_ngotos,
            forstat_has_tbc || has_forstat_upval,
            forstat_entry.nactvar,
            forstat_entry.reglevel,
        );
        fs.break_list = saved_breaklist;
    } else {
        ...
        for i in 0..(len - body_nlocals) {
            ...
            if fs.locals[idx].pidx < 0 && fs.locals[idx].kind <= RDKTOCLOSE {
                ...
                // locals[idx].name 已经是 intern 的 LuaString, 直接复用避免再次 anchor
                let varname = Some(name);
...
fn parse_func_stat(fs: &mut FuncState) {
    ...
    if chain.len() == 1 {
        ...
        if local_result.map(|(_, k)| k != VDKREG).unwrap_or(false)
            ...
            || (local_result.is_none()
                && global_kind.is_none()
                && fs.find_local_ctc(name).is_some())
        {
            fs.error(&format!("attempt to assign to const variable '{}'", name));
        }
    ...
    let mut base_reg = if fs.find_global_decl(first_name).is_some() {
        ...
        if let Some(env_reg) = fs.find_local("_ENV") {
            let r = fs.alloc_reg();
            let k = fs.string_k_ls(first_name.clone());
            code_getfield(fs, r, env_reg, k);
            r
        } else if let Some(UpvalueOrCtc::Upvalue(env_idx)) = fs.find_upvalue(&env_ls) {
            let r = fs.alloc_reg();
            let k = fs.string_k_ls(first_name.clone());
            code_gettabup(fs, r, env_idx, k);
            r
        } else {
            let r = fs.alloc_reg();
            let k = fs.string_k_ls(first_name.clone());
        ...
        reg
    } else if let Some(UpvalueOrCtc::Upvalue(uv_idx)) = fs.find_upvalue(first_name) {
        // Like C's singlevar for VUPVAL: load upvalue into a register
        let r = fs.alloc_reg();
        fs.code_abc(OpCode::GETUPVAL, r, uv_idx, 0);
        r
        ...
        // _ENV is an upvalue: use GETTABUP with correct upvalue index
        let r = fs.alloc_reg();
        let k = fs.string_k_ls(first_name.clone());
        code_gettabup(fs, r, env_idx, k);
        r
    } else {
        // Fallback: _ENV is upvalue 0 (main chunk default)
        let r = fs.alloc_reg();
        let k = fs.string_k_ls(first_name.clone());
        code_gettabup(fs, r, 0, k);
        r
    ...
    let fk = fs.string_k_ls(last_name.clone());
        ...
        if has_init {
            ...
            loop {
                ...
                if is_last && matches!(ei.exp.kind, ExpKind::Vararg) {
                    ...
                    // perf: move 替代 clone, break 后 ei.exp 不再使用
                    last_exp = Some(ei.exp);
                    n_vals += 1;
                    break;
                }
                match ei.exp.kind {
                    ...
                    _ => {
                        ...
                        // perf: move 替代 clone, 这是 ei.exp 的最后一次使用
                        last_exp = Some(ei.exp);
                    }
                }
                n_vals += 1;
            ...
            let last_is_call = last_exp
                .as_ref()
                .map(|e| matches!(e.kind, ExpKind::Call))
                .unwrap_or(false);
            if last_is_call {
            ...
            if last_is_ctc {
                ...
                let ctc_info = if last_e.kind == ExpKind::Str {
                    0
                } else {
                    last_e.info
                };
...
fn parse_constructor(fs: &mut FuncState) -> (i32, i32) {
    ...
    if !check(fs, &Token::RBrace) {
        loop {
            ...
            if !check(fs, &Token::Comma) && !check(fs, &Token::Semi) {
                break;
            }
            fs.ls_mut().next();
            if check(fs, &Token::RBrace) {
